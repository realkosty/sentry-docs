...
Environments help you better filter issues, performance metrics, releases, and user feedback in their respective pages in the Sentry UI. On each of those you can limit what you are seeing to a specific environment by picking it from the dropdown. You can also select a different default environment across all of the above views by clicking the 'Lock' button. Another area where the concept is highly useful is setting conditions that match the `environment` attribute to restrict alerts to only specific release stages.
...
Also, the environment filter affects all issue-related metrics, such as the count of users affected, time series graphs, and event count.
